...
Simulación que proporciona la distribución de encapsulados producidos en función de la densidad de células en la muestra y el tamaño de la gota.
...
def vol_droplets(vol_droplet,vol_muestra,sigma_droplet):
    ...
    while (vol_acumulado<vol_muestra):
        # En este caso el tamaño de las microgotas sigue una distribución normal.
...
def encapsulados(vol_muestra, vol_droplet, distribucion_muestra,sigma_droplet):
    """
    Función encargada de realizar los recuentos. Cuenta el número de células que ha quedado atrapado dentro de cada gota.
    """
...
def simulacion(n_celulas,vol_droplet,vol_muestra):
    """
    Realiza las simulaciones. Genera los números aleatorios y hace una llamada a las función 'encapsulados'.
    """
    ...
    [droplets,volumenes_droplets]=encapsulados(vol_muestra, vol_droplet, distribucion_muestra,sigma_droplet)
...
def aa(elementos,repeticiones,max_elementos):
    ...
    for q in range(0,len(nuevo_elementos),1):
        i=busca_elemento(elementos,q) #Busco el elemento que ocupa la posición q en el array elementos.
...
print('\n\nEl número de encapsulados múltiples representa el ',(encapsulados_multiples/encapsulados_unicos)*100,'% de los encapsulados únicos.')

print('\nEl número de encapsulados múltiples representa el',(encapsulados_multiples/encapsulados_totales)*100,'% de los encapsulados totales. Nuestro objetivo es que se encuentre en el 5%.')
